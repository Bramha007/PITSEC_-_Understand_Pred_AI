...
# Test The Classification/Regression Model On Squares Dataset
...
def _build_model(cfg: dict, device: torch.device):
    ...
    model = cls_resnet.build_resnet_classifier(model_name=model_name, out_dim=out_dim,
...
def main():
    ...
    if missing: echo_line("CLS_TEST_SD_MISSING", {"n": len(missing), "keys": missing[:8]+(["..."] if len(missing)>8 else [])})
    if unexpected: echo_line("CLS_TEST_SD_UNEXPECTED", {"n": len(unexpected), "keys": unexpected[:8]+(["..."] if len(unexpected)>8 else [])})
